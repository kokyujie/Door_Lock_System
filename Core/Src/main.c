...
char input[PASSWORD_LENGTH + 1] = {'0','0','0','0','0','0','\0'};  // Extra byte for null termination
char password[PASSWORD_LENGTH + 1] = {'1','1','1','1','1','1','\0'};  // Default password
char clear[PASSWORD_LENGTH + 1] = {'0','0','0','0','0','0','\0'};
char temp[PASSWORD_LENGTH + 1] = {'0','0','0','0','0','0','\0'};
int clearState;
int enterState;
uint8_t buttonpressedFlag = 0;
volatile uint32_t doorOpenCounter = 0;
volatile uint8_t doorOpenFlag = 0;
typedef struct {
    GPIO_TypeDef *port;
    uint16_t pin;
...
int main(void)
{
  ...
  while (1)
  {
				  ...
				  if (buttonState == GPIO_PIN_SET && ((currenttime - buttons[10].pressStartTime) >= 2000)) {
					  HD44780_Clear();
					  ...
					  for (int j = 0; j <= PASSWORD_LENGTH;){
						  ...
						  else if((enterState == 1) && (j == PASSWORD_LENGTH)){
	  ...
	  if (buttonState == GPIO_PIN_RESET){
		  for (int h = 0; h < 10; h++){
			  if (HAL_GPIO_ReadPin(buttons[h].port, buttons[h].pin) == GPIO_PIN_SET){
				  for (int k = 0; k <= PASSWORD_LENGTH;) {
					  int num2 = enter(k);
					  if(clearState == 1){
						  strcpy(input, clear);
						  k = 0;
					  ...
					  else if((enterState == 1) && (k == PASSWORD_LENGTH))	{
...
int enter(int inputindex){
	...
	while(buttonpressedFlag == 0){
		uint32_t currentTime = HAL_GetTick();
		for (int i = 0; i < NUM_INPUT; i++) { //Check for button pressed
			...
			if ((currentTime - buttons[i].lastDebounceTime) >= 50) {
				if (currentState != buttons[i].stableState) {
					buttons[i].stableState = currentState;
					if (currentState == GPIO_PIN_SET) {
						if ((i < 10) && (inputindex == 0)){
							HD44780_Clear();
						}
						if ((i < 10) && (inputindex < PASSWORD_LENGTH)){
							HD44780_PrintStr("*");
							buttonpressedFlag = 1;
							return i;
						}
						else if ((i == 10) && (inputindex > 0)){
							HD44780_Clear();
							HD44780_PrintStr("Enter Password");
							buttonpressedFlag = 1;
							clearState = 1;
...
void unlockDoor(void) {
    HAL_GPIO_WritePin(GPIOB, RELAY_PIN, GPIO_PIN_SET);  // Activate relay
    HD44780_Clear();
...
void alertDoorOpen(void){
	...
	if (HAL_GPIO_ReadPin(BUTTON_PORT, DOOR_SENSOR_PIN) == GPIO_PIN_SET || HAL_GPIO_ReadPin(BUTTON_PORT, RESET_BUTTON_PIN) == GPIO_PIN_SET){
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	if (htim->Instance == TIM3) {  // Check if TIM2 interrupt
		while (doorOpenFlag) {
			...
			else if (HAL_GPIO_ReadPin(BUTTON_PORT, DOOR_SENSOR_PIN) == GPIO_PIN_SET || HAL_GPIO_ReadPin(BUTTON_PORT, RESET_BUTTON_PIN) == GPIO_PIN_SET){
				HAL_TIM_Base_Stop_IT(&htim3);  // Stop timer interrupt when door closes
				break;
			}
		}
